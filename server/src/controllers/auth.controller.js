...
export const register = asyncHandler(async (req, res) => {
  ...
  // ============================================================
  // MLM REQUIREMENT: Referral code is MANDATORY
  // Every partner must have a valid sponsor (upline).
  // Without this rule, the tree breaks and commissions can't flow correctly.
  // ============================================================
  
  // 1. Referral code must be present
  if (!referralCode || typeof referralCode !== 'string' || referralCode.trim() === '') {
    throw new AppError(
      'Ein Empfehlungscode ist erforderlich. Bitte registrieren Sie sich über den Link Ihres Sponsors. Ohne gültigen Empfehlungscode ist keine Registrierung möglich.',
      400
    );
  }
  
  // 2. Normalize the code (trim whitespace, uppercase, strip invalid chars)
  const cleanReferralCode = referralCode.trim().toUpperCase().replace(/[^A-Z0-9_-]/g, '');
  
  if (cleanReferralCode.length < 3) {
    throw new AppError('Der Empfehlungscode ist ungültig (zu kurz).', 400);
  }
  
  // 3. Look up the upline partner by referral code
  const uplineResult = await query(
    'SELECT id, role, status, first_name, last_name FROM users WHERE UPPER(referral_code) = $1',
    [cleanReferralCode]
  );
  
  if (uplineResult.rows.length === 0) {
    throw new AppError(
      `Der Empfehlungscode "${cleanReferralCode}" wurde nicht gefunden. Bitte prüfen Sie den Code oder fragen Sie Ihren Sponsor nach einem gültigen Link.`,
      400
    );
  }
  ...
  // 4. Upline must be an active partner or admin (can't register under a suspended account)
  if (upline.status === 'suspended' || upline.status === 'cancelled') {
    throw new AppError(
      'Der Sponsor dieses Empfehlungscodes ist nicht mehr aktiv. Bitte wenden Sie sich an den Support.',
      400
    );
  }
  
  const uplineId = upline.id;
  console.log(`[REGISTER] Valid referral code "${cleanReferralCode}" → upline: ${upline.first_name} ${upline.last_name} (${upline.id})`);
  ...
  // Notify upline when a new partner joins their downline
  const adminEmail = process.env.ADMIN_EMAIL || 'service@clyr.shop';
  if (uplineId) {
    try {
      const uplineResult = await query('SELECT email, first_name FROM users WHERE id = $1', [uplineId]);
      if (uplineResult.rows[0]) {
        const upline = uplineResult.rows[0];
        const { sendEmail } = await import('../services/email.service.js');
        await sendEmail({
          to: upline.email,
          subject: `Neuer Partner in Ihrem Team: ${firstName} ${lastName}`,
          html: `<h2>Neuer Partner in Ihrem Team!</h2>
            <p>Hallo ${upline.first_name},</p>
            <p><strong>${firstName} ${lastName}</strong> (${email}) hat sich über Ihren Empfehlungslink als Partner registriert.</p>
            <p>Sie können Ihr Team im Partner-Dashboard unter "Team" einsehen.</p>`
        });
        console.log('[REGISTER] Upline notification sent to', upline.email);
      }
    } catch(emailErr) { console.error('Upline notification error:', emailErr.message); }
  }

  // ALWAYS notify admin of new registration (not conditional on upline)
  try {
    const { sendEmail } = await import('../services/email.service.js');
    await sendEmail({
      to: adminEmail,
      subject: `Neuer Partner registriert: ${firstName} ${lastName}`,
      html: `<h2>Neuer Partner registriert</h2>
        <p><strong>${firstName} ${lastName}</strong> (${email}) hat sich als Partner registriert.</p>
        <p>Land: ${country || '-'}</p>
        <p>Telefon: ${phone || '-'}</p>
        ${referralCode ? `<p>Empfehlungscode verwendet: <strong>${referralCode}</strong></p>` : '<p>Kein Empfehlungscode verwendet (direkt unter Admin eingetragen)</p>'}
        <p>Status: Ausstehend (wartet auf Bezahlung der Jahresgebühr)</p>`
    });
    console.log('[REGISTER] Admin notification sent to', adminEmail);
  } catch(emailErr) { console.error('Admin notification error:', emailErr.message); }
...
export const updateProfile = asyncHandler(async (req, res) => {
  ...
  const result = await query(
    `UPDATE users SET
     ...
     WHERE id = $14
     RETURNING *`,
    [nullIfEmpty(firstName), nullIfEmpty(lastName), nullIfEmpty(phone), nullIfEmpty(company), nullIfEmpty(vatId), nullIfEmpty(street), nullIfEmpty(zip), nullIfEmpty(city), nullIfEmpty(country), nullIfEmpty(iban), nullIfEmpty(bic), nullIfEmpty(effectiveBankName), nullIfEmpty(effectiveAccountHolder), req.user.id]
  );
