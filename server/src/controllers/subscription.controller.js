...
export const cancelSubscription = async (req, res) => {
  try {
    ...
    // Notify customer and admin about cancellation (best effort)
    try {
      ...
      if (details?.customer_email) {
        await sendEmail({
          ...
          html: `
            ...
            <p>Hallo ${details.customer_first_name || ''},</p>
            <p>Ihr Abonnement${details.product_name ? ` fuer <strong>${details.product_name}</strong>` : ''} wurde gekuendigt.</p>
            <p>${reason ? `Grund: ${reason}` : ''}</p>
          `
        }).catch((e) => console.error('Subscription cancellation customer email failed:', e.message));

        const adminEmail = process.env.ADMIN_EMAIL || process.env.SMTP_FROM || 'service@clyr.shop';
        await sendEmail({
          to: adminEmail,
          subject: `Abo gekuendigt: ${details.customer_first_name || ''} ${details.customer_last_name || ''}`.trim(),
          html: `
            <h2>Abo-Kuendigung</h2>
            <p><strong>Kunde:</strong> ${details.customer_first_name || ''} ${details.customer_last_name || ''}</p>
            <p><strong>E-Mail:</strong> ${details.customer_email}</p>
            <p><strong>Produkt:</strong> ${details.product_name || details.product_id}</p>
            <p>${reason ? `<strong>Grund:</strong> ${reason}` : ''}</p>
          `
        }).catch((e) => console.error('Subscription cancellation admin email failed:', e.message));
      }
    } catch (mailErr) {
      console.error('Subscription cancellation notification error:', mailErr.message);
    }
