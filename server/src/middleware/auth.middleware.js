...
export const authenticateCustomer = async (req, res, next) => {
  try {
    ...
    // Check if it's a customer token - support both field names
    const isCustomer = decoded.role === 'customer' || decoded.type === 'customer';
    if (!isCustomer) {
      return res.status(401).json({ 
        error: 'Nicht autorisiert',
        message: 'Ungueltiger Kundentoken' 
      });
    }
    ...
    req.user = { id: result.rows[0].id, role: 'customer' };
    next();
  } catch (error) {
    if (error.name === 'TokenExpiredError') {
      return res.status(401).json({ 
