...
async function seed() {
  ...
  try {
    ...
    const ranks = [
      ...
      { name: 'Direktor', slug: 'direktor', level: 7, commission_rate: 31, min_own_sales: 0, min_team_sales: 0, min_direct_partners: 0, one_time_bonus: 0, color: '#7C3AED', description: 'Administratorrang - nur für Geschäftsführung' },
    ];
    ...
    // Theresa earns 31% as R6 Sales Manager - confirmed via actual March 2026 payouts
    ...
    // Also ensure ALL existing admin users get Sales Manager rank (not Direktor)
    await client.query(`UPDATE users SET rank_id = $1 WHERE role = 'admin'`, [salesManagerRankId]);
