...
export const getCommissionSummary = async (userId) => {
  ...
  // VAT info for display (do not alter wallet/commission totals)
  const userResult = await query(
  ...
  return {
    totalEarned: toNumber(result.rows[0].total_earned),
    pending: toNumber(result.rows[0].pending),
    available: toNumber(result.rows[0].available),
    paidOut: toNumber(result.rows[0].paid_out),
    thisMonth: toNumber(result.rows[0].this_month),
