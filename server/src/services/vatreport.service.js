...
export const generateVatReport = async (country, reportType, periodStart, periodEnd) => {
  ...
  try {
    ...
    console.log('[VAT] vat_reports table: OK');
  } catch(e) {
    console.error('[VAT] vat_reports table MISSING:', e.message);
    throw new Error('vat_reports Tabelle fehlt — Server neu starten');
  }
  ...
  const ordersResult = await query(`
    SELECT 
      ...
      o.payment_status,
      o.created_at,
      o.billing_country as customer_country,
      o.customer_vat_id,
      COALESCE(c.country, o.billing_country) as cust_country,
      COALESCE(c.vat_id, o.customer_vat_id) as cust_vat_id
    FROM orders o
  ...
  if (orders.length > 0) {
    console.log('[VAT] Sample order columns:', Object.keys(orders[0]).join(', '));
    console.log('[VAT] Sample order:', JSON.stringify({
      order_number: orders[0].order_number,
      vat: orders[0].vat,
      vat_amount: orders[0].vat_amount,
      vat_rate: orders[0].vat_rate,
