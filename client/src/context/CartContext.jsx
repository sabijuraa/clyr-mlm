...
export const CartProvider = ({ children }) => {
  ...
  const addItem = useCallback((product, quantity = 1) => {
    setItems(currentItems => {
      // Build a unique cart key: product id + selected variants
      // This allows same product with different aromas/variants as separate cart lines
      const variantKey = product.selectedVariants
        ? Object.values(product.selectedVariants).map(v => v?.id || v?.name || '').sort().join('|')
        : '';
      const cartKey = `${product.id}__${variantKey}`;

      console.log('[CART] addItem:', product.name, 'cartKey:', cartKey, 'variants:', product.selectedVariants);

      const existingIndex = currentItems.findIndex(item => item.cartKey === cartKey);
      ...
      console.log('[CART] Adding new line item with cartKey:', cartKey);
      return [...currentItems, {
        id: product.id,
        cartKey,                          // unique per variant combination
        name: product.name,
        slug: product.slug,
        price: product.price,
        images: images || [],
  ...
  const removeItem = useCallback((cartKey) => {
    // Support both cartKey (new) and productId (legacy)
    setItems(currentItems => currentItems.filter(item => 
      item.cartKey !== cartKey && item.id !== cartKey
    ));
  }, []);
  ...
  // Check if product is in cart - supports both cartKey (variant-aware) and productId (legacy)
  const isInCart = useCallback((keyOrId) => {
    return items.some(item => item.cartKey === keyOrId || item.id === keyOrId);
  }, [items]);

  // Get quantity of specific product in cart
  const getItemQuantity = useCallback((keyOrId) => {
    const item = items.find(item => item.cartKey === keyOrId || item.id === keyOrId);
