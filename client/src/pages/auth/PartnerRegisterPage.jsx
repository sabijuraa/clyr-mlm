...
const PartnerRegisterPage = () => {
  ...
  const nextStep = async () => {
    ...
    if (step === 1) {
      fieldsToValidate = ['referralCode', 'firstName', 'lastName', 'email', 'phone', 'password'];
    } else if (step === 2) {
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-secondary-700 mb-2">
                          {lang === 'de' ? 'Empfehlungscode' : 'Referral Code'} <span className="text-red-500">*</span>
                        </label>
                        <input
                          {...register('referralCode', { 
                            required: lang === 'de' 
                              ? 'Empfehlungscode ist erforderlich. Bitte verwenden Sie den Link Ihres Sponsors.' 
                              : 'Referral code is required. Please use your sponsor\'s link.',
                            minLength: {
                              value: 3,
                              message: lang === 'de' ? 'Code ist zu kurz' : 'Code too short'
                            }
                          })}
                          className={inputClass(errors.referralCode)}
                          placeholder="z.B. THERESA"
                          readOnly={!!referralCode}
                        />
                        {errors.referralCode && <p className="mt-1 text-sm text-red-500">{errors.referralCode.message}</p>}
                        {!referralCode && (
                          <p className="mt-1 text-xs text-amber-600">
                            {lang === 'de' 
                              ? '⚠️ Sie müssen einen gültigen Empfehlungscode von Ihrem Sponsor angeben.' 
                              : '⚠️ You must provide a valid referral code from your sponsor.'}
                          </p>
                        )}
                        {referralCode && (
                          <p className="mt-1 text-xs text-green-600">
                            ✓ {lang === 'de' ? 'Empfehlungscode wurde automatisch übernommen' : 'Referral code automatically applied'}
                          </p>
                        )}
                      </div>
