            ...
            >
              {detailLoading ? (
                ...
                <>
                  ...
                  {/* Team Tree View */}
                  {partnerDetail.teamTree?.length > 0 && (
                    <div className="px-6 pb-6">
                      <h3 className="text-sm font-semibold text-secondary-500 uppercase mb-3">Team-Struktur</h3>
                      <div className="border border-gray-100 rounded-xl p-4 bg-gray-50 max-h-64 overflow-y-auto">
                        {partnerDetail.teamTree.map((member) => (
                          <div key={member.id} className="flex items-center gap-3 py-2 border-b border-gray-100 last:border-0"
                            style={{ paddingLeft: `${((member.depth || 1) - 1) * 20 + 8}px` }}>
                            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${member.status === 'active' ? 'bg-green-400' : 'bg-gray-300'}`} />
                            <span className="text-sm font-medium text-secondary-700">{member.first_name} {member.last_name}</span>
                            <span className="text-xs text-secondary-400 ml-auto">{member.rank_name}</span>
                            <span className="text-xs text-secondary-400">Ebene {member.depth}</span>
                          </div>
                        ))}
                      </div>
                    </div>
