  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5">
        <h3 className="font-semibold text-gray-900 mb-3 text-sm sm:text-base">Muster-Vorschau</h3>
        <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3">
          <button onClick={async () => {
            try {
              const res = await api.get('/admin/invoices/sample/customer', { responseType: 'blob' });
          ...
          <button onClick={async () => {
            try {
              const res = await api.get('/admin/invoices/sample/reverse-charge', { responseType: 'blob' });
              const url = URL.createObjectURL(new Blob([res.data], { type: 'application/pdf' }));
              window.open(url, '_blank');
            } catch(e) { toast.error('Fehler beim Laden'); }
          ...
          <button onClick={async () => {
            try {
              const res = await api.get('/admin/invoices/sample/commission', { responseType: 'blob' });
              const url = URL.createObjectURL(new Blob([res.data], { type: 'application/pdf' }));
              window.open(url, '_blank');
            } catch(e) { toast.error('Fehler beim Laden'); }
          ...
          <div className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 -mx-1 px-1">
            {['all', 'customer', 'commission', 'fees'].map((type) => (
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-lg transition text-xs sm:text-sm whitespace-nowrap flex-shrink-0 ${
                  filter === type 
                    ? 'bg-primary-500 text-white' 
                    : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                {type === 'all' ? 'Alle' : type === 'customer' ? 'Kunden' : type === 'commission' ? 'Provisionen' : 'Gebuehren'}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <table className="w-full min-w-[600px]">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase">Partner</th>
                  <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase hidden sm:table-cell">Rang</th>
                  <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase">Betrag</th>
                  <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase hidden md:table-cell">Zahlungsart</th>
                  <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase hidden lg:table-cell">Zeitraum</th>
                  <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase">Status</th>
                  <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase hidden sm:table-cell">Datum</th>
                  <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 uppercase">PDF</th>
                </tr>
              </thead>
                ...
                {feePayments.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    <td className="py-3 px-3">
                      ...
                      <p className="text-xs text-gray-500 truncate max-w-[120px]">{p.email}</p>
                    </td>
                    <td className="py-3 px-3 hidden sm:table-cell">
                      <span className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded-full text-xs font-medium">{p.rank_name || '-'}</span>
                    </td>
                    <td className="py-3 px-3 font-semibold text-gray-900 text-sm">{formatCurrency(p.amount)}</td>
                    <td className="py-3 px-3 text-sm text-gray-600 hidden md:table-cell">{p.payment_method || 'Stripe'}</td>
                    <td className="py-3 px-3 text-sm text-gray-600 hidden lg:table-cell">
                  ...
                  <tr key={invoice.id} className="hover:bg-gray-50">
                    <td className="px-3 py-3 whitespace-nowrap">
                      <div className="flex items-center">
                        <FileText className="w-4 h-4 text-gray-400 mr-1.5 flex-shrink-0" />
                    ...
                    <td className="px-3 py-3">
                      <div>
                        <p className="text-gray-900 text-sm truncate max-w-[140px]">
                    ...
                    <td className="px-3 py-3 whitespace-nowrap font-medium text-gray-900 text-sm">
