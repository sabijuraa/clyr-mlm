...
const AdminOrdersPage = () => {
    ...
    try {
      ...
      setOrders(ordersList.map(o => ({
        ...
        partner: o.partner_first_name ? {
          ...
          code: o.partner_code || o.referral_code || ''
        } : null,
        shippingAddress: {
  ...
  const stats = {
    ...
    revenue: orders.filter(o => o.paymentStatus === 'paid' && o.status !== 'cancelled').reduce((sum, o) => sum + o.total, 0)
  };
  ...
  const getPaymentConfig = (status) => {
    const configs = {
      ...
      pending: { label: 'Zahlung ausstehend', icon: Clock, color: 'text-amber-500 font-semibold' },
      failed: { label: 'Zahlung fehlgeschlagen', icon: XCircle, color: 'text-red-500 font-semibold' },
      refunded: { label: 'Erstattet', icon: XCircle, color: 'text-secondary-500' },
      partially_refunded: { label: 'Teilw. erstattet', icon: XCircle, color: 'text-secondary-500' },
    };
    return configs[status] || configs.pending;
  };
  ...
  const openOrderDetails = async (order) => {
    ...
    try {
      const res = await ordersAPI.getById(order.rawId || order.id);
      ...
      setSelectedOrder({
        ...
        customer: {
          ...
          email: fullOrder.customer_email || order.customer.email || ''
        },
        shippingAddress: {
          street: fullOrder.shipping_street || fullOrder.billing_street || order.shippingAddress.street || '',
          city: fullOrder.shipping_city || fullOrder.billing_city || order.shippingAddress.city || '',
          zip: fullOrder.shipping_zip || fullOrder.billing_zip || order.shippingAddress.zip || '',
          country: fullOrder.shipping_country || fullOrder.billing_country || order.shippingAddress.country || ''
        }
      });
    } catch (e) {
      setSelectedOrder(order);
      toast.error('Bestelldetails konnten nicht vollständig geladen werden');
    } finally {
              ...
              return (
                <div key={order.id} className="p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="font-semibold text-secondary-700 text-sm">{order.id}</p>
                      <p className="text-xs text-secondary-500">{formatDate(order.createdAt)}</p>
                    </div>
                    <p className="font-bold text-secondary-700">{formatCurrency(order.total)}</p>
                  </div>
                  <p className="text-sm text-secondary-600 mb-2">{order.customer.firstName} {order.customer.lastName}</p>
                  <div className="flex items-center gap-2 flex-wrap mb-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${statusConfig.color}`}>
                      {order.status}
                    </span>
                    <span className={`inline-flex items-center gap-1 text-xs font-medium ${paymentConfig.color}`}>
                      {paymentConfig.label}
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => openOrderDetails(order)}
                      className="flex-1 text-xs py-1.5 px-3 border border-gray-200 rounded-lg text-secondary-600 hover:bg-gray-100">
                      Details
                    </button>
                  </div>
                </div>
      ...
      >
        {selectedOrder && (
          <div className="space-y-6">
            {loadingDetails && (
              <div className="text-sm text-secondary-500">Details werden geladen...</div>
            )}
            {/* Status Row */}
            ...
            <div>
              ...
              <div className="p-4 bg-gray-50 rounded-xl space-y-2">
                {selectedOrder.items?.length > 0 ? selectedOrder.items.map((item, idx) => (
                  <div key={item.id || idx} className="flex justify-between gap-3 text-sm">
                    <div className="text-secondary-700">
                      {item.quantity}x {item.product_name}
                      {getVariantText(item) ? (
                        <span className="text-secondary-500 italic"> ({getVariantText(item)})</span>
                      ) : null}
                    </div>
                    <div className="font-semibold text-secondary-700">{formatCurrency(item.total || 0)}</div>
                  </div>
                )) : (
                  <div className="text-sm text-secondary-500">Keine Produktdetails vorhanden.</div>
                )}
                {selectedOrder.trackingNumber && (
                  <div className="pt-2 mt-2 border-t border-gray-200 text-xs text-secondary-500">
            ...
            <div className="flex gap-3 pt-4 border-t border-gray-100 flex-wrap">
              {selectedOrder.paymentStatus !== 'paid' && (
                <Button variant="primary" className="flex-1" icon={CreditCard}
                  onClick={async () => {
                    if (!confirm(`Bestellung ${selectedOrder.id} manuell als bezahlt markieren? Provisionen werden automatisch berechnet.`)) return;
                    try {
                      await api.post(`/orders/${selectedOrder.rawId}/mark-paid`, { note: 'Manuelle Bestätigung durch Admin' });
