...
import { Users, UserPlus, Search, Download, Lightbulb, RefreshCw, ChevronDown, ChevronUp, Mail, Phone, MapPin, Calendar } from 'lucide-react';
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        {viewMode === 'tree' ? (
          <>
            ...
            <TeamTree data={filteredTree} currentUser={user} isLoading={isLoading} />
          </>
        ) : (
          <>
            <h3 className="font-heading font-semibold text-lg text-secondary-700 mb-6">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filteredList.map((m, idx) => {
                      ...
                      const isExpanded = expandedId === (m.id || idx);
                      return (
                        <React.Fragment key={m.id || idx}>
                        <tr className="hover:bg-gray-50 cursor-pointer"
                          onClick={() => setExpandedId(isExpanded ? null : (m.id || idx))}>
                          <td className="py-3">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                <div className="flex items-start gap-2">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-secondary-700 break-all">{m.email || '-'}</p>
                                  </div>
                                </div>
                                {m.phone && (
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-secondary-700">{m.phone}</p>
                                    </div>
                                  </div>
                                )}
                                {(m.street || m.city) && (
                                  <div className="flex items-start gap-2 sm:col-span-2">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-secondary-700">
                                        ...
                                        {m.zip} {m.city}{m.country && `, ${m.country}`}
                                      </p>
                                    </div>
                                  </div>
                                )}
                                {m.birth_date && (
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-secondary-700">{new Date(m.birth_date).toLocaleDateString('de-DE')}</p>
                                    </div>
                                  </div>
                                )}
                                {m.company && (
                                  <div>
                                    ...
                                    <p className="text-secondary-700">{m.company}</p>
                                  </div>
                                )}
                                {m.vat_id && (
                                  <div>
                                    <p className="text-xs text-gray-400">USt-IdNr.</p>
                                    <p className="text-secondary-700">{m.vat_id}</p>
                                  </div>
                                )}
                                <div>
                                  <p className="text-xs text-gray-400">Registriert</p>
                                  <p className="text-secondary-700">{m.created_at ? new Date(m.created_at).toLocaleDateString('de-DE') : '-'}</p>
                                </div>
                                <div>
                                  <p className="text-xs text-gray-400">Provisionsrate</p>
                                  <p className="text-secondary-700">{m.commission_rate || m.commissionRate || '-'}%</p>
                                </div>
                              </div>
                            </td>
                          </tr>
