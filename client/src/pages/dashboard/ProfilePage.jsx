...
const ProfilePage = () => {
  ...
  const handleStripeConnect = async () => {
    ...
    try {
      const r = await api.post('/stripe-connect/onboarding');
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          {activeTab === 'bank' && (
            <div className="space-y-6">
              ...
              <div className={`p-4 rounded-xl border ${stripeStatus?.connected && stripeStatus?.payoutsEnabled ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-semibold text-sm text-secondary-700">Automatische Auszahlungen via Stripe</p>
                    {stripeStatus?.connected && stripeStatus?.payoutsEnabled ? (
                      <p className="text-sm text-green-700 mt-1">✅ Verbunden — Auszahlungen werden automatisch am 1. des Monats überwiesen</p>
                    ) : stripeStatus?.connected ? (
                      <p className="text-sm text-amber-700 mt-1">⚠️ Konto verbunden aber noch nicht vollständig eingerichtet</p>
                    ) : (
                      <p className="text-sm text-amber-700 mt-1">⚠️ Noch nicht verbunden — Bitte Stripe-Konto einrichten für automatische Auszahlungen</p>
                    )}
                  </div>
                  {stripeStatus?.connected && stripeStatus?.payoutsEnabled ? (
                    <button onClick={async () => { const r = await api.get('/stripe-connect/dashboard-link'); window.open(r.data.url, '_blank'); }}
                      className="px-3 py-1.5 text-xs bg-green-600 text-white rounded-lg hover:bg-green-700">
                      Stripe Dashboard
                    </button>
                  ) : (
                    <button onClick={handleStripeConnect} disabled={connectingStripe}
                      className="px-3 py-1.5 text-xs bg-[#635BFF] text-white rounded-lg hover:bg-[#4F46E5] disabled:opacity-50">
                      {connectingStripe ? 'Weiterleitung...' : 'Mit Stripe verbinden'}
                    </button>
                  )}
                </div>
              </div>
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl text-sm text-blue-800">
                <strong>Hinweis:</strong> Auszahlungen erfolgen automatisch via Stripe am 1. des Monats auf Ihr Bankkonto.
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
