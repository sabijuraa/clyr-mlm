...
export default function CheckoutPage() {
  ...
  const verifyReferralCodeFn = async (code) => {
    ...
    try {
      // Pass userId so backend can block self-referral
      const userId = localStorage.getItem('userId') || user?.id || null;
      const response = await referralAPI.verify(codeToCheck, userId);
      ...
      if (msg.includes('eigenen')) {
        toast.error('Sie können nicht Ihren eigenen Empfehlungscode verwenden.');
      }
      setReferralValid(false);
      setReferralPartner(null);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Ensure items have integer productId, preserve variant info per line
      const orderItems = effectiveCartItems.map(item => ({
        productId: parseInt(item.id || item.productId, 10),
        quantity: parseInt(item.quantity, 10) || 1,
        cartKey: item.cartKey || `${item.id}__`,
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className={`bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg shadow border-2 ${referralValid ? 'border-green-400' : 'border-red-400'}`}>
                ...
                <p className="text-sm text-gray-700 mb-4">
                  CLYR Produkte sind ausschließlich über Partner erhältlich. Bitte geben Sie den Empfehlungscode Ihres Partners ein, um fortzufahren.
                </p>
                {!referralValid && (
                  <p className="text-sm text-red-600 font-medium mb-3">
                    ⚠️ Ein gültiger Empfehlungscode ist erforderlich.
                  </p>
                )}
