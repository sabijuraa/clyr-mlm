    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="bg-slate-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <nav className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm overflow-x-auto whitespace-nowrap">
            <Link to="/" className="text-secondary-500 hover:text-primary-500 flex-shrink-0">Home</Link>
            <ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-secondary-400 flex-shrink-0" />
            <Link to="/products" className="text-secondary-500 hover:text-primary-500 flex-shrink-0">{lang === 'de' ? 'Produkte' : 'Products'}</Link>
            <ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-secondary-400 flex-shrink-0" />
            <span className="text-secondary-700 font-medium truncate">{product.name}</span>
          </nav>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="min-w-0">
              <div className="relative aspect-square max-w-full bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl sm:rounded-3xl overflow-hidden mb-4">
                {product.is_new && (
                  <span className="absolute top-3 left-3 sm:top-4 sm:left-4 z-10 px-2 sm:px-3 py-0.5 sm:py-1 bg-primary-500 text-white text-[10px] sm:text-xs font-bold rounded-full">{lang === 'de' ? 'NEU' : 'NEW'}</span>
                )}
                {product.is_featured && (
                  <span className="absolute top-3 right-3 sm:top-4 sm:right-4 z-10 px-2 sm:px-3 py-0.5 sm:py-1 bg-secondary-700 text-white text-[10px] sm:text-xs font-bold rounded-full">BESTSELLER</span>
                )}
                <img
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              <div>
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-secondary-700 mb-2 break-words">{product.name}</h1>
              ...
              {totalPrice > 0 ? (
                ...
                <div className="text-xl sm:text-2xl font-bold text-primary-600">
                  ...
                  <span className="text-xs sm:text-sm font-normal text-secondary-500 ml-2 block mt-1">
                          ...
                          return (
                            ...
                            >
                              ...
                              {option.priceModifier > 0 && (
                                <span className="ml-2 text-xs sm:text-sm text-green-600">+{formatCurrency(option.priceModifier)}</span>
                              )}
                            </button>
                          );
                        })}
